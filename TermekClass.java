...
public TermekClass(){ Termek = new ArrayList<>(); }
    ...
    public int GetTeremByname(String tname) { 
        ...
        if (tname.length()>0){
            for (i=0;i< Termek.size();i++) {
                if (Termek.get(i).GetTeremName().equalsIgnoreCase(tname)) { out = i; break; }
            }
        }
        return out;
    }  
     public boolean AddTerem(TeremType type, String TeremName ,int MaxSzam, int Egyeb) { 
        ...
        } else { System.out.println("Nem engedek több termet!"); } 
        return out;
    }
      ...
      public void AllTeremInfo() { 
        int i;
        if (Termek.size() == 1 ) { System.out.println("Csak egy terem van megadva:)"); }
        else if (Termek.size() > 1 ) { System.out.println("Termek adatai"); }
        else { System.out.println("Nincsenek teredm adatok!"); }
