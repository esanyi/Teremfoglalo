...
public class Beadando1 {
    ...
    public static void main(String[] args) {
      ...
      TeremCount=0;
      NL="";
      while ( FC.hasNextLine() && NL.length()==0) { NL = FC.GetLine().trim(); }
          if ( NL.length() > 0 ) {
                    try {
                ...
                } else { System.out.println(ERR_FILE_FORMAT); }
           ...
           while ( FC.hasNextLine() ) {
                       NL = removeDuplicateWhitespace(FC.GetLine().trim());
                        if (NL.length() > 0 ) {
                            ...
                            } else { System.out.println("Hibás sor: " + NL); } 
                            if (i+1==TeremCount) { break; } 
                        }
                    }
      ...
      } else { System.out.println("Nem létezik a megadott fájl"); }
         } 
    }
}
