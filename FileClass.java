...
public final class FileClass {
    ...
    public boolean hasOpenedFile() {
        if (br != null && FileName != null && FileName.length() > 0) {
            return true;
        } else {
            return false;
        }
        ...
        if (FN.length() > 0) {
            try {
                ...
                if (f.exists() == false) {
                    if (CreateNew == true) {
                        f.createNewFile();
                        System.out.println("'" + FN + "' fájl létrehozása");
                    }
                } else {
                    System.out.println("'" + FN + "' fájl megnyitása");
                }
                if (f.canRead() == true) {
    ...
    public FileClass(String FN, boolean CreateNew) {
        OpenFile(FN, CreateNew);
    }
